...
```{r}
Primary7MinC.plot <- ggplot(data = df, aes(x = PrecipSum8_22, y = Primary7, color = Sex))+
  geom_jitter(width = 0.2, shape = 1, size = 1.8)+
  geom_smooth(method="lm", aes(fill = Sex), alpha = 0.2)+
  scale_color_manual(name="",
                    values=c("#E69F00","#56B4E9","#000000"),
  ...
  theme(plot.title = element_text(size = 16),
          ...
          legend.position = "top",
          plot.margin = margin(15,15,15,15))
suppressWarnings(print(Primary7MinC.plot))
```
```{r A.Primary7.mdl}
A.Primary7.mdl <- lm(Primary7 ~ MinC8_22 * PrecipSum8_22 + Weight, data = scaled.df)
summary(A.Primary7.mdl)
```

```{r A.WingChord.mdl}
A.WingChord.mdl <- lm(WingChord ~ MinC8_22 * PrecipSum8_22 + Weight, data = scaled.df)
...
TBSWeightRatioMinC.plot <- ggplot(data = df, aes(x=MinC8_22, y = TBSWeightRatio, color = Sex))+
  geom_jitter(width = 0.2, shape = 1, size = 1.8)+
  geom_smooth(method="lm", aes(fill = Sex), alpha = 0.2)+
  scale_color_manual(name="",
                    values=c("#E69F00","#56B4E9","#000000"),
                    labels=c("Female","Male","Unsexed"))+
  scale_fill_manual(values = c("#E69F00","#56B4E9","#000000"), guide = "none")+
  ylab("Total bill surface area/weight ratio")+
  xlab("Average minimum temperature (C)")+
  theme(plot.title = element_text(size = 16),
          axis.text.x = element_text(size = 12),
          axis.title.x = element_text(size = 14),
