...
This csv file contains morphological data on 2030 American Crow nestlings, measured between 1989-2022. The first 26 vectors represent the nestling data; the rest of the vectors (27- 46) contain data on the average minimum and maximum temperatures (C) and summed precipitation (cm) for different combinations of days between each nestling's hatch date and 22 days after the hatch date.

For example, MaxC1_11 represents the average maximum temperature for each nestling's first 11 days post hatching.

We used an R package called `climwin` to identify the climate metric and combination of days that explains the most variation in nestling growth, essentially identifying the range of days when the nestlings grew the most. We selected the best fit models for different climate metrics and different day ranges by comparing the models' Akaike's Information Criterion (AIC) values. AIC model comparison considers the number of estimated parameters (parsimony) and the maximum likelihood function for each model.

In this method of model comparison, models with lower AIC values explain more variation in the data set than models with higher AIC values.

The climate variable and day range that explained the most variation in nestling growth was the average minimum temperature between and 8-22 days post hatching.
...
This csv file holds weather data from one Cornell University's weather stations. It includes data from 1989-2022.
...
We then subset the full data frame by sex to allow for modeling of the sexes separately.
...
As expected, nestling weight increases linearly with nestling age.
...
Next, we'll investigate whether climate affects nestling weight by plotting the relationship and then modeling it to see if any trends (patterns) that we see in the plot are statistically significant.
...
It looks like increasing minimum temperatures have a significant negative effect on nestling weight (for males, females and all nestlings combined), suggesting that there may be some trade off that affects nestling growth during years with warmer temperatures.

Now let's model the relationship for males, females, and all nestlings separately. Because these variables are continuous, we can stick with a simple linear model for data that trend towards a gaussian (normal) distribution.
...
Forest plots are a popular way to show model results in figure form. The y-axis shows the different parameters of the model, and the x-axis represents the parameter's estimated effect on nestling weight. The points are the model estimates; the error bars are the 95% confidence intervals of the estimates. If the error bars do not intersect the dotted red line at zero, then the parameter's effect is statistically significant.

## Nestling WingChord

```{r WingChord.df}
WingChord.df <- subset(df, WingChord > 0 & WingChord < 221)
scaled.WingChord.df <- subset(scaled.df, WingChord > -8.5 & WingChord < 6.5 )
M.scaled.WingChord.df <- subset(scaled.WingChord.df, Sex == "M")
F.scaled.WingChord.df <- subset(scaled.WingChord.df, Sex == "F")
```

```{r WingChordPrecip.plot}
WingChordPrecip.plot <- ggplot(data = WingChord.df, aes(x = PrecipSum8_22, y = WingChord, color = Sex))+
  geom_jitter(width = 0.2, shape = 1, size = 1.8)+
  geom_smooth(method="lm", aes(fill = Sex), alpha = 0.2)+
  scale_color_manual(name="",
                    values=c("#E69F00","#56B4E9","#000000"),
  ...
  theme(plot.title = element_text(size = 16),
          ...
          legend.position = "top",
          plot.margin = margin(15,15,15,15))
suppressWarnings(print(WingChordPrecip.plot))
```

### Male WingChord Model

```{r M.WingChord.mdl}
M.WingChord.mdl <- lm(WingChord ~ MinC8_22 * PrecipSum8_22 + Weight, data = M.scaled.WingChord.df)
summary(M.WingChord.mdl)
```

### Female WingChord Model

```{r F.WingChord.mdl}
F.WingChord.mdl <- lm(WingChord ~ MinC8_22 * PrecipSum8_22 + Weight, data = F.scaled.WingChord.df)
summary(F.WingChord.mdl)
```

### All Nestlings WingChord Model

```{r A.WingChord.mdl}
A.WingChord.mdl <- lm(WingChord ~ MinC8_22 * PrecipSum8_22 + Weight, data = scaled.WingChord.df)
summary(A.WingChord.mdl)
```

### Data frames for WingChord Forest Plots

```{r WingChord.dfs for fplot}
M.WingChord.df <- fplot_df(M.WingChord.mdl, "Male", "WingChord")
F.WingChord.df <- fplot_df(F.WingChord.mdl, "Female", "WingChord")
A.WingChord.df <- fplot_df(A.WingChord.mdl, "All", "WingChord")

WingChord.fplot.df <- rbind(M.WingChord.df, F.WingChord.df, A.WingChord.df)
```

### WingChord Forest Plot

```{r WingChord.fplot}
WingChord.fplot <- fplot(WingChord.fplot.df)
WingChord.fplot
#cowplot::save_plot("Plots/WingChord.fplot.png", WingChord.fplot, base_height = 4, base_width = 7)
```

### WingChord Plots Wrapped

```{r WingChord.wrap}
WingChord.wrap <- ggarrange(WingChordPrecip.plot, WingChord.fplot, ncol = 2, labels = c("A","B"), widths = c(1.6,1.4))
suppressWarnings(print(WingChord.wrap))
cowplot::save_plot("PaperPlots/WingChord.wrap.png", WingChord.wrap, base_height = 5, base_width = 10)
```
...
TBSMinC.plot <- ggplot(data = df, aes(x = MinC8_22, y = TBS, color = Sex))+
  ...
  scale_color_manual(name="",
                    ...
                    labels=c("Female","Male","Unknown"))+
  scale_fill_manual(values = c("#E69F00","#56B4E9","#000000"), guide = "none")+
  labs(x = "Average minimum temperature (C)", y = bquote('Total bill surface area scaled'  ~(mm^2)))+
  theme(plot.title = element_text(size = 16),
          axis.text.x = element_text(size = 12),
          axis.title.x = element_text(size = 14),
          axis.text.y = element_text(size = 14),
...
## Nestling TBS/Weight Ratio

```{r TBSWeightRatioMinC.plot}
TBSWeightRatioMinC.plot <- ggplot(data = scaled.df, aes(x=MinC8_22, y = TBSWeightRatio, color = Sex))+
  geom_jitter(width = 0.2, shape = 1, size = 1.8)+
  geom_smooth(method="lm", aes(fill = Sex), alpha = 0.2)+
  scale_color_manual(name="",
                    values=c("#E69F00","#56B4E9","#000000"),
                    labels=c("Female","Male","Unsexed"))+
  scale_fill_manual(values = c("#E69F00","#56B4E9","#000000"), guide = "none")+
  ylab("Scaled total bill surface area/weight ratio")+
  xlab("Average minimum temperature scaled (C)")+
  theme(plot.title = element_text(size = 16),
          axis.text.x = element_text(size = 12),
          axis.title.x = element_text(size = 14),
          ...
          legend.position = "top",
          plot.margin = margin(15,15,15,15))
suppressWarnings(print(TBSWeightRatioMinC.plot))
```

### Male TBSWeightRatio Model

```{r M.TBSWeightRatio.mdl}
M.TBSWeightRatio.mdl <- lm(TBSWeightRatio ~ MinC8_22 * PrecipSum8_22, data = M.scaled.df)
summary(M.TBSWeightRatio.mdl)
```

### Female TBSWeightRatio Model

```{r F.TBSWeightRatio.mdl}
F.TBSWeightRatio.mdl <- lm(TBSWeightRatio ~ MinC8_22 * PrecipSum8_22, data = F.scaled.df)
summary(F.TBSWeightRatio.mdl)
```

### All Nestlings TBSWeightRatio Model

```{r A.TBSWeightRatio.mdl}
A.TBSWeightRatio.mdl <- lm(TBSWeightRatio ~ MinC8_22 * PrecipSum8_22, data = scaled.df)
summary(A.TBSWeightRatio.mdl)
```

### Data frames for TBSWeightRatio Forest Plot

```{r TBSWeightRatio.dfs for fplot}
M.TBSWeightRatio.df <- fplot_df(M.TBSWeightRatio.mdl, "Male", "TBSWeightRatio")
F.TBSWeightRatio.df <- fplot_df(F.TBSWeightRatio.mdl, "Female", "TBSWeightRatio")
A.TBSWeightRatio.df <- fplot_df(A.TBSWeightRatio.mdl, "All", "TBSWeightRatio")

TBSWeightRatio.df <- rbind(M.TBSWeightRatio.df, F.TBSWeightRatio.df, A.TBSWeightRatio.df)
```

### TBSWeightRatio Forest Plot

```{r TBSWeightRatio.fplot}
TBSWeightRatio.fplot <- fplot(TBSWeightRatio.df)
TBSWeightRatio.fplot
#cowplot::save_plot("Plots/TBSWeightRatio.fplot.png", TBSWeightRatio.fplot, base_height = 4, base_width = 7)
```

### TBS Plots Wrapped

```{r TBSWeightRatio.wrap}
TBSWeightRatio.wrap <- ggarrange(TBSWeightRatioMinC.plot, TBSWeightRatio.fplot, ncol = 2, labels = c("A","B"), widths = c(1.6,1.4))
suppressWarnings(print(TBSWeightRatio.wrap))
cowplot::save_plot("PaperPlots/TBSWeightRatio.wrap.png", TBSWeightRatio.wrap, base_height = 5, base_width = 10)
```
