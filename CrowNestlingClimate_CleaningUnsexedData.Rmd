...
df$ID[df$ID=="dead fledgling USGS16"]#not in
...
df <- df %>%  group_by(Year) %>% arrange(NestName, CalcAge) %>% dplyr::distinct(ID, .keep_all = TRUE)#new n=2339 (19 fewer than above, so code works)
