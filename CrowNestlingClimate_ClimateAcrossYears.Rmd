...
clim.df <- read.csv("ClimateVariables.csv", h=TRUE)
clim.df <- clim.df %>% 
  filter(Month <6)
```

```{r plot median max temp}
median(clim.df$MaxTempC, na.rm=TRUE)
max.plot <- ggplot(data = clim.df, aes(x=as.factor(Year), y=MaxTempC))+
  geom_boxplot()+
  geom_hline(yintercept = 12.78, color = "red", linetype = "dotted", linewidth = 1)+
  #ggtitle("Daily Maximum Temperatures across March, April, May")+
...
median(clim.df$MinTempC, na.rm=TRUE)
min.plot <- ggplot(data = clim.df, aes(x=as.factor(Year), y=MinTempC))+
  geom_boxplot()+
  geom_hline(yintercept = 1.11, color = "red", linetype = "dotted", size = 1)+
  #ggtitle("Daily Minimum Temperature across March, April, May")+
...
```{r plot precip}
median(clim.df$PrecipCM, na.rm=TRUE)
...
annotate_figure(climate.wrap, top = "Climate Conditions across March, April, May")
```
