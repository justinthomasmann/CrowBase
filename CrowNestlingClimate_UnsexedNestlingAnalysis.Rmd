...
df$CLessH1_11 <- df$CDDSum1_11-df$HDDSum1_11
df$CLessH12_22 <- df$CDDSum12_22-df$HDDSum12_22
```
<br>
...
```{r BD Time 2 model}
BDTime2.scaled.mdl <- lm(data = B.scaled.df, BD ~ GDDSum12_22 * PrecipSum12_22 + Weight + CalcAge)
summary(BDTime2.scaled.mdl)
```


```{r bill depth by CLessH scaled}
ggplot(data = B.scaled.df, aes(x=CLessH12_22,y=BD,color=AllSex))+
  geom_point()+
  geom_smooth()+
...
How to deal with the sex differences in the unsexed kids:
1. Classify clusters of years that are climatically similar.
2. 
