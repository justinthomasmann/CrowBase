...
```{r Weight interplot}
Weight.intplot <- plot_model(MinC8_22.Weight.mdl, type = "int", mdrt.values = "quart")
Weight.intplot
Weight.intplot <- Weight.intplot +
  scale_color_manual(name="Precipitation Quartiles", values=c("#FFC107","#1E88E5","#D81B60"), breaks =  c(-0.75,-0.26,0.58), labels=c("Lower","Median","Upper"))+
...
Weight.intplot
cowplot::save_plot("Plots/Weight.intplot.png", Weight.intplot, base_height = 4, base_width = 6)
```
...
```{r WeightbyMin.plot}
WeightbyMin.plot <- ggplot(data = df, aes(x = MinC8_22, y = Weight))+
  geom_point(shape = 1)+
  geom_smooth()+
  ylab("Weight (g)")+
  xlab("Averaged minimum temperature (C)")
WeightbyMin.plot
cowplot::save_plot("Plots/WeightbyMin.plot.png", WeightbyMin.plot, base_height = 4, base_width = 7)
```

```{r WeightbyGDD.plot}
WeightbyGDD.plot <- ggplot(data = df, aes(x = GDD10CSum8_22, y = Weight))+
  geom_point(shape = 1)+
  geom_smooth()+
  ylab("Weight (g)")+
  xlab("Summed growing degree days (10 C threshold)")
WeightbyGDD.plot
cowplot::save_plot("Plots/WeightbyGDD.plot.png", WeightbyGDD.plot, base_height = 4, base_width = 7)
```
...
```{r UBSWeightRatiobyMin.mdl}
MinC8_22.UBSWeightRatio.mdl <- lm(UBSWeightRatio ~ MinC8_22 * PrecipSum8_22 + Weight, data = scaled.df, na.action = "na.fail")
summary(MinC8_22.UBSWeightRatio.mdl)
```



```{r UBSWeightRatiobyMin.plot}
B.df$UBSWeightRatio <- B.df$UBS/B.df$Weight
UBSWeightRatiobyMin.plot <- ggplot(data = B.df, aes(x = MinC8_22, y = UBSWeightRatio, color= AllSex))+
  geom_point(shape = 1)+
  geom_smooth()+
  #scale_y_continuous(limits = c(0.1,0.9))+
  ylab("Upper bill surface area/weight ratio")+
  xlab("Average minimum temperature (C)")
cowplot::save_plot("Plots/UBSWeightRatiobyMin.plot.png", UBSWeightRatiobyMin.plot, base_height = 4, base_width = 6)
```
