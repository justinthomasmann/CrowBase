...
GDD.BD.dredge <- dredge(GDD10C.BD.mdl)
GDD.BD.dredge
```

```{r}
GDD10C.BD.MinInter.mdl <- lm(BD ~ GDD10CSum12_22 + MinC12_22 * PrecipSum12_22 + Weight, data = scaled.df, na.action = "na.fail")

GDD10C.BD.GDDInter.mdl <- lm(BD ~ MinC12_22 + GDD10CSum12_22 * PrecipSum12_22 + Weight, data = scaled.df, na.action = "na.fail")
```
```{r}
Inter.BD.mdls <- list(GDD10C.BD.MinInter.mdl,GDD10C.BD.GDDInter.mdl)
Inter.BD.modnames <- c("MinInter","GDDInter")
...
ggplot(data = MinC.pred, aes(x,predicted))+
  geom_point(data = scaled.df, aes(x=MinC12_22, y=BD), shape = 1)+
...
```
```{r BD climate metric best model plot}
ggplot(data = MinC8_22.pred, aes(x,predicted))+
  geom_point(data = scaled.df, aes(x=MinC8_22, y=BD))+
  geom_line(color = "red")+
  geom_ribbon(aes(ymin=conf.low,ymax=conf.high), fill = "red", alpha = 0.1)+
  scale_y_continuous(limits = c(-5,5))+
  ylab("Bill depth")+
  xlab("8-22 days post-hatching minimum temperature (C) ")
```
