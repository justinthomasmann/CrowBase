...
max.plot <- ggplot(data = clim.df, aes(x=as.factor(Year), y=MaxTempC))+
  geom_boxplot()+
  geom_hline(yintercept = 16.67, color = "red", linetype = "dotted", size = 1)+
...
min.plot <- ggplot(data = clim.df, aes(x=as.factor(Year), y=MinTempC))+
  geom_boxplot()+
  geom_hline(yintercept = 3.89, color = "red", linetype = "dotted", size = 1)+
...
climate.wrap <- ggarrange(min.plot,max.plot,precip.plot, ncol = 1)
annotate_figure(climate.wrap, top = "Climate Conditions across March, April, May and June")
```
