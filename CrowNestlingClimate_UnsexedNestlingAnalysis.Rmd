...
Exclusions, nestlings with abnormal measurements 
...
```{r Exclusions.df}
Decisions.df <- read.csv("CrowNestlingClimateAllMeasures-Exclusions_WOSJun23A.csv", h=TRUE)

Exclusions.df <- Decisions.df %>% 
  filter(Decision==1) %>% 
  select(Decision,ID) 

Decisions.df <- Decisions.df %>% filter(Decision > 1)
```
...
```{r write CrowNestlingClimate_NestlingsCleaned&ClimateJoined_7.1.23.csv}
#write.csv(df, "CrowNestlingClimate_NestlingsCleaned&ClimateJoined_7.1.23.csv", row.names = FALSE)
```
...
```{r WeightAge.plot}
WeightAge.plot <- ggplot(data = df, aes(x = CalcAge, y = Weight))+
  geom_point(shape = 1)+
  geom_smooth()+
  ylab("Weight (g)")+
  xlab("Nestling Age")
...
## Upper Bill Surface Area (UBS) by sex

```{r UBSSex.mdl}
MinC8_22.UBSSex.mdl <- lm(UBS ~ AllSex + MinC8_22 * PrecipSum8_22 + Weight, data = B.scaled.df)
summary(MinC8_22.UBSSex.mdl)
```

```{r UBSSex climate time period best model predition}
UBSSex.pred <- ggpredict(MinC8_22.UBSSex.mdl, type = "fe", terms = ~MinC8_22)
UBSSex.pred
```

```{r UBSSex climate metric best model plot}
UBSbyMin.plot <- ggplot(data = B.scaled.df, aes(x=MinC8_22, y=UBS/sqrt(Weight), color=AllSex))+
  geom_point(shape = 1)+
...
TarsusPrecipSex.plot <- ggplot(data = B.scaled.df, aes(x=PrecipSum8_22, y=Tarsus, color=AllSex))+
  geom_point(shape = 1)+
  geom_smooth(alpha = 0.2)+
  scale_color_manual(values = c("#009E73","#E69F00"), labels = c("Female","Male"))+
  labs(color = "Sex")+
...
```{r Tarsus.wrap}
Tarsus.wrap <- ggarrange(Tarsus.fplot,TarsusPrecipSex.plot, ncol = 2, labels = c("A","B"), widths = c(1.3,1.7))
...
summary(MinC8_22.BNT.mdl)
```

```{r BNT climate time period best model predition}
BNT.pred <- ggpredict(MinC8_22.BNT.mdl, type = "fe", terms = ~MinC8_22)
BNT.pred
```

```{r BNT climate metric best model plot}
BNTbyMin.plot <- ggplot(data = BNT.pred, aes(x,predicted))+
  geom_point(data = scaled.df, aes(x=MinC8_22, y=BNT, labels=ID), shape = 1)+
  geom_line(color = "#1E88E5", size = 0.7)+
  geom_ribbon(aes(ymin=conf.low,ymax=conf.high), fill = "#1E88E5", alpha = 0.2)+
  ylab("Bill nares to tip (mm) scaled")+
  xlab("Mean minimum temperature (C) scaled")
BNTbyMin.plot
#ggplotly(BNTbyMin.plot)
#cowplot::save_plot("Plots/BNTbyMin.plot.png", BNTbyMin.plot, base_height = 4, base_width = 6)
```

```{r BNT interaction plot}
BNTInter.plot <- plot_model(MinC8_22.BNT.mdl, type = "int", mdrt.values = "quart")+
  scale_color_manual(name="Precipitation Quartiles",
                    values=c("#D81B60","#FFC107","#1E88E5"),
                    breaks =  c(-0.77,-0.22,0.54),
                    labels=c("Lower","Median","Upper"))+
  ...
  theme(plot.title = element_text(size = 16),
          ...
          legend.position = c(0.3,0.8))
BNTInter.plot
#cowplot::save_plot("Plots/BNTInter.plot.png", BNTInter.plot, base_height = 4, base_width = 6)
```

```{r BNT MinC8_22.mdl fplot.df}
BNT.df <- tidy(MinC8_22.BNT.mdl, quick = FALSE)
  ci <- as_tibble(confint(MinC8_22.BNT.mdl))
  BNT.df$ci.lower <- ci$`2.5 %`
  BNT.df$ci.upper <- ci$`97.5 %`
  BNT.df$Metric <- "Bill nares to tip"
  BNT.df <- BNT.df[-1,]
```
...
```{r BNT.wrap}
BNT.wrap <- ggarrange(BNT.fplot,BNTbyMin.plot, ncol = 2, labels = c("A","B"), widths = c(1.3,1.7))
cowplot::save_plot("Plots/BNT.wrap.png", BNT.wrap, base_height = 5, base_width = 12)
```
