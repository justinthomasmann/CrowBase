...
Here, we'll compare AIC values from models using MinC, MaxC, GDD10C, MaxCMinus10C and OneCMinusMinC measurements for days 12-22. 
...
MaxCMinus10C.BD.mdl <- lm(BD ~ MaxCMinus10C12_22 * PrecipSum12_22 + Weight, data = scaled.df, na.action = "na.fail")

OneCMinusMinC.BD.mdl <- lm(BD ~ OneCMinusMinC12_22 * PrecipSum12_22 + Weight, data = scaled.df, na.action = "na.fail")
```

```{r bill depth climate metric model comparison}
BDTime2Models <- list(MinC.BD.mdl,MaxC.BD.mdl,GDD10C.BD.mdl,MaxCMinus10C.BD.mdl,OneCMinusMinC.BD.mdl)
BDTime2ModelNames <- c('MinC','MaxC','GDD10C','MaxCMinus10C','OneCMinusMinC')
aictab(cand.set = BDTime2Models, modnames = BDTime2ModelNames, sort = TRUE)
```

MinC Model fits the data best; GDD is second best. But the delta AICc is 11.28 between the top two models. Comparing the model summaries below, precipitation has a significant negative affect in the MinC model but not the GDD10C model. Additionally, MinC has a slightly higher estimate but also a slightly higher std. error.
...
Using MinC as the best fit metric from above, here we'll look at whether days 1-11, 12-22, or average of the best sliding windows, days 6-19
```{r bill depth time period models}
MinC1_11.BD.mdl <- lm(BD ~ MinC1_11 * PrecipSum12_22 + Weight, data = scaled.df, na.action = "na.fail")

MinC12_22.BD.mdl <- lm(BD ~ MinC12_22 * PrecipSum12_22 + Weight, data = scaled.df, na.action = "na.fail")

MinC.win.BD.mdl <- lm(BD ~ MinC.win * PrecipSum12_22 + Weight, data = scaled.df, na.action = "na.fail")

MinC8_22.BD.mdl <- lm(BD ~ MinC8_22 * PrecipSum12_22 + Weight, data = scaled.df, na.action = "na.fail")

```

```{r bill depth time period model comparison}
BDTimeModels <- list(MinC1_11.BD.mdl,MinC12_22.BD.mdl,MinC.win.BD.mdl,MinC8_22.BD.mdl)
BDTimeModelNames <- c('MinC1_11','MinC12_22','MinC.win',"MinC8_22")
aictab(cand.set = BDTimeModels, modnames = BDTimeModelNames, sort = TRUE)
```
...
MaxCMinus10C.BW.mdl <- lm(BW ~ MaxCMinus10C12_22 * PrecipSum12_22 + Weight, data = scaled.df, na.action = "na.fail")

MinC8_22.BW.mdl <- lm(BW ~ MinC8_22 * PrecipSum12_22 + Weight, data = scaled.df, na.action = "na.fail")
```

```{r bill width time 2 model comparison}
BWTime2Models <- list(MinC.BW.mdl,MaxC.BW.mdl,GDD10C.BW.mdl,MaxCMinus10C.BW.mdl,MinC8_22.BW.mdl)
BWTime2ModelNames <- c('MinC','MaxC','GDD10C','MaxCMinus10C','MinC8_22')
aictab(cand.set = BWTime2Models, modnames = BWTime2ModelNames, sort = TRUE)
```
