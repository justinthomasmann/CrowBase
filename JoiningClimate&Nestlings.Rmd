...
```{r add hatch dates for mouth color birds}
...
```{r proto.df}
proto.df <- read.csv("NestlingMDatesUpdated.csv", h=TRUE)
#include only nestlings with a hatch date
proto.df <- proto.df %>% 
  filter(!is.na(hatchDate))
proto.df$hatchDateJul <- as.integer(proto.df$hatchDateJul)
class(proto.df$hatchDateJul)
```

```{r clim.df}
clim.df <- read.csv("ClimateMetrics.csv", h=TRUE)

df <- left_join(proto.df,clim.df, by = "hatchDateJul")
df <- df %>% 
  filter(year!=1989)
df <- df %>% 
  select(!c(observation, nestlingMeasurementID,population,species,
            lifeStage,measureNumber,Obs,MaxTemp,Precip,CDD,GDD,))
#write.csv(df, "Nestlings&ClimateProto.csv")
```
